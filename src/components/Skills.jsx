...
const GROUPS = [
    ...
    { title: '// Frameworks & Libraries', delay: '0.05s', pills: ['Spring Boot','Spring MVC','Spring Security','JPA / Hibernate','RESTful APIs','JWT Auth','FastAPI','Apache POI','JavaMailSender'].map(l => ({ label: l, cls: 'p-cyan' })) },
    { title: '// Databases',              delay: '0.1s',  pills: ['MySQL','PostgreSQL','MongoDB','Oracle'].map(l => ({ label: l, cls: '' })).concat([{ label: 'Redis', cls: 'p-orange' }]) },
    { title: '// DevOps & Tools',         delay: '0.15s', pills: [{ label: 'Docker', cls: 'p-orange' }, ...['Git / GitHub','Maven','Postman','Swagger','Cloudinary','Spring Scheduler'].map(l => ({ label: l, cls: '' }))] },
